...
public class RouteeSmsService implements SmsService {
    ...
    public String getAccessToken() {
        ...
        headers.setBasicAuth(encoded); // Χρήση της έτοιμης μεθόδου για Basic Auth
        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);

        HttpEntity<String> request = new HttpEntity<>("grant_type=client_credentials", headers);

        ResponseEntity<Map> response = this.restTemplate.postForEntity(AUTHENTICATION_URL, request, Map.class);

        if (response.getBody() != null) {
            return (String) response.getBody().get("access_token");
        }
        throw new RuntimeException("Failed to obtain Routee Access Token");
    }
